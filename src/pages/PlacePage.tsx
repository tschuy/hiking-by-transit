...
export function PlacePage({ slug }: { slug: string }) {
  ...
  return <article className="page container place-page">
    <nav className="breadcrumbs" aria-label="Breadcrumb"><a href="/">Home</a><span aria-hidden="true">/</span>{parent && <><a href={`/guides/${parent.slug}`}>{parent.title}</a><span aria-hidden="true">/</span></>}<span>{place.title}</span></nav>
    <p className="page-context">{kindLabels[place.kind]}</p><h1>{place.title}</h1>{place.blurb && <p className="page-lede">{place.blurb}</p>}
    ...
    {guides.length > 0 && <aside className="place-guide-callout"><h2>Getting to and around {place.title}</h2>{guides.map((guide) => <a className="button-link" href={getGuidePath(guide.slug)} key={guide.guide_id}>{guide.title} <span aria-hidden="true">→</span></a>)}</aside>}
    {children.length > 0 && <section className="place-section" aria-labelledby="within-place"><h2 id="within-place">Guides within {place.title}</h2><nav className="region-list" aria-label={`Guides within ${place.title}`}>{children.map((child) => <a href={`/guides/${child.slug}`} key={child.place_id}>{child.title}<span aria-hidden="true"> →</span></a>)}</nav></section>}
    ...
    {ownedDestinations.length === 0 && mapScope && <section className="place-section" aria-labelledby="place-map"><h2 id="place-map">Trailheads in {place.title}</h2><TrailheadMap key={place.place_id} scope={place.place_id} center={mapScope.center} transitGroups={mapScope.transitGroups} defaultTransitGroups={mapScope.defaultTransitGroups} label={place.title} /></section>}
  </article>
}

...
import { getCatalogTrailheadById } from '../data/trailheadCatalog'
...
export function HikePage({ slug }: { slug: string }) {
  ...
  const trailheads = hike.trailhead_ids.map(getCatalogTrailheadById).filter((trailhead) => trailhead !== undefined)
  ...
  return <article className="page container hike-page">
    ...
    {place && <p className="page-context"><a href={`/guides/${place.slug}`}>{place.title}</a></p>}<h1>{hike.title}</h1>{hike.blurb && <p className="page-lede">{hike.blurb}</p>}
    {hike.image && <img className="hike-hero-image" src={`/assets/${hike.image}`} alt={`Landscape along ${hike.title}`} />}
    <ul className="summary-grid hike-summary" aria-label="Hike summary"><li><strong>{hike.length}</strong><span>Distance</span></li><li><strong>{difficulty}</strong><span>Difficulty</span></li><li><strong>{hike.travel.served === 'daily' ? 'All week' : hike.travel.served}</strong><span>Transit service</span></li></ul>
    ...
    <section className="transit-itinerary" aria-labelledby="transit-title"><h2 id="transit-title">From {hike.travel.origin}</h2><div className="itinerary-grid"><TravelLeg title="To the trail" leg={hike.travel.out} />{hike.travel.return && <TravelLeg title="Return trip" leg={hike.travel.return} />}</div>{hike.travel.origin && <p className="field-note">Travel times are estimates from {hike.travel.origin}.</p>}</section>
    ...
    {hike.gpx ? <GpxMap file={hike.gpx} title={hike.title} trailheads={trailheads} /> : <p className="notice"><em>This legacy guide does not yet include a GPX route.</em></p>}
  </article>
}

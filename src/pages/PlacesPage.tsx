...
export function PlacesPage() {
  ...
  return <section className="page container"><h1>Places</h1><p className="page-lede">Explore beyond. Learn how to visit California's National Parks and famous outdoor destinations.</p><div className="places-grid">{visiblePlaceContent.map((place, index) => <a className={`editorial-card region-theme-${index % 3 + 1}`} href={`/places/${place.slug}`} key={place.place_id}><span className="card-context">{parentName(place.parent_id) ?? place.kind}</span><strong>{place.title}</strong><span>{place.blurb ?? firstBodyParagraph(place.body) ?? `${place.kind} guide`}</span><b>Explore this place <span aria-hidden="true">→</span></b></a>)}</div></section>
}

...
export function PostPage({ date, slug }: { date: string; slug: string }) {
  ...
  return <article className="page container post-page"><nav className="breadcrumbs" aria-label="Breadcrumb"><a href="/">Home</a><span aria-hidden="true">/</span><a href="/posts">Posts</a><span aria-hidden="true">/</span><span>{post.title}</span></nav><h1>{post.title}</h1><time className="post-date" dateTime={post.date}>{formatDate(post.date)}</time><PostBody markdown={post.body} title={post.title} /></article>
}
